...
BOMojo_IMDB_edited1 <- BOMojo_IMDB_edited %>%
  ...
  select(-5, -6)
BOMojo_IMDB_edited1 <- merge(BOMojo_IMDB_edited1, Fill_in_IMDB_data, by.x = c("Title2", "Year"), by.y = c("Title", "BOMojo year"), all = TRUE)
...
BOMojo_IMDB_edited3 <- BOMojo_IMDB_edited2 %>%
  filter(!(is.na(IMDB_ID))) %>%
  select(-6)
```
